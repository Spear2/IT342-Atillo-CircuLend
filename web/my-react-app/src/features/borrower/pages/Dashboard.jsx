...
import ActiveBorrowedItems from "@/features/borrower/components/ActiveBorrowedItems";
import TransactionHistory from "@/features/borrower/components/TransactionHistory";
import Footer from "@/shared/layout/Footer";
import { apiFetch } from "@/shared/utils/apiFetch";
import { getAuthHeader } from "@/shared/security/auth";
...
export default function BorrowerDashboard() {
  ...
  const loadTransactions = async () => {
    ...
    try {
      ...
      if (err.code === "AUTH-002" || err.status === 401) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setReturnSuccess(""), 3000);
  ...
  const confirmReturn = async () => {
    ...
    try {
      ...
      } else if (err.code === "DB-001") {
        setReturnError("Transaction not found.");
      } else {
  ...
  return (
    <div className="dashboard-wrapper">
      ...
      <main className="dashboard-container">
        ...
        {returnSuccess && <div className="dashboard-alert dashboard-alert-success">{returnSuccess}</div>}
